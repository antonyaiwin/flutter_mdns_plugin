...
public class FlutterMdnsPlugin implements FlutterPlugin, MethodCallHandler {
  ...
  private void startDiscovery(String serviceName) {
    ...
    mNsdManager = (NsdManager)mBinding.getApplicationContext().getSystemService(Context.NSD_SERVICE);
  ...
  public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
    ...
    EventChannel serviceDiscoveredChannel = new EventChannel(mBinding.getBinaryMessenger(), NAMESPACE + "/discovered");
    mDiscoveredHandler = new ServiceDiscoveredHandler();
    serviceDiscoveredChannel.setStreamHandler(mDiscoveredHandler);

    EventChannel serviceResolved = new EventChannel(mBinding.getBinaryMessenger(), NAMESPACE + "/resolved");
    mResolvedHandler = new ServiceResolvedHandler();
    serviceResolved.setStreamHandler(mResolvedHandler);

    EventChannel serviceLost = new EventChannel(mBinding.getBinaryMessenger(), NAMESPACE + "/lost");
    mLostHandler = new ServiceLostHandler();
    serviceLost.setStreamHandler(mLostHandler);

    EventChannel discoveryRunning = new EventChannel(mBinding.getBinaryMessenger(), NAMESPACE + "/running");
    mDiscoveryRunningHandler = new DiscoveryRunningHandler(mBinding.getApplicationContext());
